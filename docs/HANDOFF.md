...
- Synced fork `main` with `upstream/main` (`alexmmatos/arthur-mcp`) a second time same-day, merging in `2f77099` ("fix mcp problems" — the `resources/templates/list` + JSON-RPC normalization entry directly below). Backed up pre-merge state to `backup/pre-upstream-sync-20260722-2` (the earlier same-day sync already used the plain `-20260722` name). Conflicts, resolved by combining both sides rather than picking one: `docs/HANDOFF.md`/`docs/ROADMAP.md` (kept both sides' entries, upstream's on top as the incoming commit's work); `PromptTestPanel.tsx`/`ResourceTestPanel.tsx` (upstream's removal of the `id` field from example JSON-RPC payloads combined with this fork's local edits in the same regions). Validated: see below once run.
- Added `resources/templates/list` as a fully supported MCP method (it was previously unimplemented, so clients calling it during discovery got `Method not found`):
  - `DynamicMcpService.createMcpServer` (`api/src/dynamic-mcp/dynamic-mcp.service.ts`) now registers a handler for `ListResourceTemplatesRequestSchema` returning `{ resourceTemplates: [] }`. Arthur resources always have a fixed URI (no `{param}` placeholders), so there are no templates to return yet — the handler exists so the method itself is real rather than absent. No server capability flag change needed: resource templates live under the existing `resources: {}` capability already declared.
  - Found while investigating that the `mcpResourcesTemplatesList`/`mcpResourcesTemplatesListDesc` locale keys already existed in `en`/`pt-BR` `servers.json` but were never wired to a UI element. Added the matching `DocAccordion` + simulator panel to the public Share page's "MCP Protocol" reference section (`src/pages/SharePage/SharePage.tsx`), between `resources/list` and `prompts/list`, sending `{ jsonrpc: '2.0', method: 'resources/templates/list', params: {} }` like the other protocol simulators.
  - Also documented the method in `docs/manifest.proto.txt`'s `mcpEndpoints.methods` list, alongside the other JSON-RPC methods.
  - Validation: verified `ListResourceTemplatesRequestSchema.parse({ method: 'resources/templates/list' })` and `ListResourceTemplatesResultSchema.parse({ resourceTemplates: [] })` both succeed against the vendored SDK; `npx tsc --noEmit -p api/tsconfig.json` clean; `npm run type-check` (frontend) clean; `npx jest dynamic-mcp` — 64/66 passing, the 2 failures are in `response-mapper.spec.ts` and are the same pre-existing unrelated failures already noted in this file's `SQLITE_ERROR` entry.
- Fixed created MCP servers returning nothing (no `initialize` result, no tools/resources/prompts) for AI/agent clients that send minimal or incomplete JSON-RPC bodies — down to a bare `{ "method": "initialize" }` with no `jsonrpc`, `id`, or `params` at all:
  - Root cause (three layered issues in the `@modelcontextprotocol/sdk` transport, all hit by real AI/agent clients that don't send a spec-complete JSON-RPC envelope):
    1. A message without `jsonrpc: "2.0"` fails the SDK's message parsing outright (`400 Parse error`).
    2. A message without `id` still matches the SDK's notification shape (`JSONRPCNotificationSchema`, which doesn't require `id`), so it's silently reclassified as a notification and gets an empty `202` — no body, no error — for every method, including `initialize`, `tools/list`, `resources/list`, and `prompts/list`.
    3. `initialize` specifically requires `params.protocolVersion` (string), `params.capabilities` (object), and `params.clientInfo` (`{ name, version }`) — the SDK's own `Server` validates incoming params against `InitializeRequestSchema` before running its handler, so a request missing any of them gets a real but unhelpful JSON-RPC error instead of a handshake result.
    All three were confirmed directly against the vendored SDK source (`types.js`, `shared/protocol.js`, `server/index.js`, `server/webStandardStreamableHttp.js`) and by round-tripping a normalized bare `{ method: 'initialize' }` payload through the SDK's own `JSONRPCRequestSchema`/`InitializeRequestSchema` `.parse()`.
  - Fix: `DynamicMcpController.handlePost` (`api/src/dynamic-mcp/dynamic-mcp.controller.ts`) now runs every incoming body through `normalizeJsonRpcBody` before it reaches the SDK transport. For any request-shaped message (has `method`, not `notifications/*`) it backfills `jsonrpc: "2.0"` and a synthetic `id` (`randomUUID()`) when missing; for `initialize` specifically it also backfills `params.protocolVersion` (SDK's own `DEFAULT_NEGOTIATED_PROTOCOL_VERSION`), `params.capabilities` (`{}`), and `params.clientInfo` (`{ name: 'unknown-client', version: '0.0.0' }`) when missing or malformed, preserving any values the client did provide. Real notifications are left untouched.
  - Also removed the `id` field from every example/"try it out" JSON-RPC payload the frontend builds — `ToolAccordion`, `EndpointAccordion`, `ResourceTestPanel`, `PromptTestPanel`, `McpDocs`, `SharePage`'s simulator panels, and `buildMcpCurl.util.ts` — so the documented/copyable examples match what a minimal client (and the backend's new tolerance) actually needs; these payloads are shared between display and the live "execute" call, so this is safe now that missing `id` is backfilled server-side.
  - Documentation synchronized in `docs/FLOWS.md` ("MCP Tool Execution (dynamic-mcp)" risk-to-preserve list and the "Public MCP Share Page" simulator note).
  - Validation: `api/src/dynamic-mcp/dynamic-mcp.controller.spec.ts` (6/6, covers jsonrpc/id backfill, existing-value passthrough, real notifications, batched messages, bare-initialize params backfill, and partial-initialize-params preservation), `npx tsc --noEmit -p api/tsconfig.json` clean, `npm run type-check` (frontend structure + template catalogs + `tsc --noEmit`) clean. Also manually verified the normalized bare-initialize output against the SDK's own `JSONRPCRequestSchema`/`InitializeRequestSchema` via a throwaway script — both parse successfully.
...
- **Production incident: accidental bulk-delete of every Deployment in the cluster via Headlamp, recovered via direct `kubectl`/SSH.** The user selected and deleted all Deployments across all namespaces from the Headlamp UI. Impact: CoreDNS, metrics-server, local-path-provisioner, and Traefik (kube-system); cert-manager + cainjector + webhook (cert-manager); source-controller, kustomize-controller, helm-controller, notification-controller, image-reflector/automation-controller, source-watcher (flux-system); and the `arthur-mcp` app itself — full outage. DaemonSets (`svclb-traefik`), Secrets (`arthur-mcp-secrets`, `arthur-mcp-tls`), Certificates, and CRDs were untouched (Headlamp's bulk action only targeted Deployments), so no credential/cert regeneration was needed. Recovery, in dependency order: (1) re-applied the untouched k3s-native manifests already on the VPS at `/var/lib/rancher/k3s/server/manifests/{coredns.yaml,local-storage.yaml,metrics-server/}` — these restored CoreDNS/local-path-provisioner/metrics-server directly, no Helm involved; (2) deleted the already-`Complete` `helm-install-traefik`/`helm-install-traefik-crd` Jobs in `kube-system` so k3s's built-in helm-controller re-ran them against the still-present `HelmChart` CRs, recreating Traefik; (3) re-applied the upstream `cert-manager.yaml` for the exact version already in use (v1.21.0, read off the `certificates.cert-manager.io` CRD's `app.kubernetes.io/version` label) since no `helm`/`flux` CLI or cached manifests existed on the host; (4) re-applied the latest `fluxcd/flux2` `install.yaml` to bring the controllers back — once `kustomize-controller`/`source-controller` were running again, Flux's own `arthur-mcp-production` `Kustomization` reconciled from the already-synced `GitRepository` and recreated the `arthur-mcp` Deployment/ConfigMap/ServiceAccount/Ingress/NetworkPolicy/Middleware with no manual manifest work. **Gap surfaced**: there is no on-host or off-cluster backup of the Flux bootstrap manifests (no `flux`/`helm` CLI on the VPS, no cached `install.yaml`, nothing under `deploy/` in this repo) — recovery depended on `kubectl apply -f <upstream release URL>` reaching GitHub at incident time. Consider vendoring `flux install.yaml` and `cert-manager.yaml` under `deploy/` (or documenting the exact bootstrap command) so a future recovery doesn't depend on picking a compatible version blind. Also consider restricting the Headlamp/kubeconfig identity used day-to-day to something short of cluster-admin bulk-delete, or requiring per-namespace scoping in that tool.

- **Added a HorizontalPodAutoscaler capping the app at 5 replicas** (`deploy/k8s/base/hpa.yaml`, registered in the base kustomization): `minReplicas: 1`, `maxReplicas: 5`, scaling on 80% average CPU utilization (the container already declares `requests.cpu: 250m`; k3s ships metrics-server). Removed the fixed `replicas: 1` from the Deployment so `kubectl apply` doesn't fight the HPA. Not yet applied to the cluster — needs `kubectl apply -k deploy/k8s/overlays/production`. Note: `RateLimitGuard`'s 60 req/min window is in-memory per pod, so at 5 replicas the effective per-server ceiling is up to 300 req/min; acceptable for now, revisit if the limit must be global. Validated: `kubectl kustomize deploy/k8s/overlays/production` renders the HPA with the right target.

- **Applied the mobile-first logo treatment to the auth pages the retrofit (80f9c19) missed**: `Signup`, `ForgotPassword`, and `ResetPassword` still used the unconstrained logo block (`height: '100%', maxWidth: '100%'`), letting the image render at natural size and blow out the card on small screens. They now use the same constrained block as `Login` (wrapper `height={112}` + `overflow="hidden"`, image `width: '100%', height: 'auto', maxWidth: 356`). No flow, copy, or permission change. Validated: `npm run type-check` clean, `npx vitest run src/pages/Signup src/pages/Login` 8/8.

- **Fixed "Server not found" on `/servers/:id` against pre-existing databases**: the `AddOAuthConfigToSwaggerProjects1700000000002` migration file existed (with passing spec) but was never registered in `TYPEORM_MIGRATIONS` (`api/src/database/database.module.ts`), and the `oauth_config` column had only been added retroactively to the already-executed initial migration — so databases created before the OAuth-config feature never got the column. `findById` selects all entity columns → SQL error → 500 → the frontend maps any fetch error to "Server not found". The list endpoint worked because `findAll` uses an explicit column select that omits `oauth_config` (which is also why production, running a pre-OAuth build, was unaffected). Fix: registered the migration (idempotent via `hasColumn`); it applies on next backend boot (`migrationsRun: true`). Lesson recorded: never edit an already-shipped migration — new columns need a new registered migration. Validated: `npx jest add-oauth-config-to-swagger-projects.spec` 1/1, `npm run build --prefix api` clean.
