...
- Synced fork `main` with `upstream/main` (`alexmmatos/arthur-mcp`), merging in `959572d` ("fix server detail icon" — see the two entries directly below). Backed up pre-merge state to `backup/pre-upstream-sync-20260722`. Two conflicts, both resolved by combining both sides rather than picking one: `docs/HANDOFF.md` (kept both sides' "Latest Changes" entries — upstream's moved to the top as the incoming commit's work); `src/pages/ServerDetail/ServerDetail.tsx` (upstream's `getProjectIcon(project, templateSummaries)` icon fix + this fork's `mcpServerIdentifier` line both kept — `templateSummaries` was already in scope from the same incoming commit's other hunk). Added a repo-scoped `sync-upstream` Claude Code skill (`.claude/skills/sync-upstream/SKILL.md`) documenting this exact procedure for future syncs. Validated: `npm run type-check` clean, frontend `npm test` 108/108, backend `npm test --prefix api` 252/253 (1 pre-existing opt-in PostgreSQL integration test skipped, as expected without a live DB).
