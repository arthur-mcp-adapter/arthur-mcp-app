...
- [x] Synced fork `main` with `upstream/main`, merging `2f77099` ("fix mcp problems" — the two entries directly below). See `docs/HANDOFF.md`.
- [x] Added `resources/templates/list` as a real, supported MCP method (previously unimplemented → `Method not found`): `DynamicMcpService` now returns an empty `resourceTemplates` list (Arthur resources have fixed URIs, no template placeholders yet), and the Share page's "MCP Protocol" reference section gained the matching accordion/simulator using the `mcpResourcesTemplatesList*` locale keys that already existed but were unused. See `docs/HANDOFF.md`.
- [x] Fixed created MCP servers returning empty/broken `initialize`/`tools/list`/`resources/list`/`prompts/list` responses for AI/agent clients sending minimal JSON-RPC bodies (down to a bare `{ "method": "initialize" }`): `DynamicMcpController` now backfills `jsonrpc`, `id`, and (for `initialize`) `params.protocolVersion`/`capabilities`/`clientInfo` on any request-shaped message missing them, before the SDK ever sees it. Every frontend example/simulator payload had its now-unneeded `id` field removed to match. See `docs/HANDOFF.md` for details.
