...
- [x] Fixed Server Detail MCP tests end to end: Tool, Resource, Prompt, API Endpoint, curl, and AI View requests now use the server `shareSlug`, while MCP authentication `401` responses remain scoped to the test result instead of invalidating the Supabase application session.
