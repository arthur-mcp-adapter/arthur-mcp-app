...
- **Profile self-edit** (`Profile` → `MyProfileTab`): `supabase.auth.updateUser({ email, password, data: { username } })` directly — no `currentPassword` re-entry (the live session is the credential), and an email change requires confirming the new address before it takes effect. After username metadata changes, the frontend calls `supabase.auth.refreshSession()` so the replacement access token carries the updated `user_metadata.username`; otherwise `GET /users/me` would continue returning the old username from the stale JWT until its natural refresh.
- **Session sync**: `AuthProvider` subscribes to `supabase.auth.onAuthStateChange()` as the single source of truth, mirroring the current access token into `localStorage['token']` (which `src/api.ts` reads for application API requests) and re-fetching `GET /users/me`. The Supabase bearer is never attached to `/mcp/*`, whose API key/OAuth credentials form an independent authentication domain. `logout()` and the application-session 401 interceptor in `src/api.ts` both call `supabase.auth.signOut()`, not just clear the mirrored token — otherwise Supabase's own session store resurrects it on reload. The interceptor waits for sign-out to finish before navigating and deduplicates concurrent 401 handling so a rejected session cannot create a reload loop. A `401` from `/mcp/*` is excluded because it represents independent MCP authentication; tool, resource, and prompt test failures remain visible in the test panel without signing the Arthur user out.
...
Deploy the configured UI before enabling CAPTCHA protection in Supabase to avoid locking out all
password flows.
...
- Server Detail uses the project UUID for administrative `/swagger/servers/:id` operations, but its Tool, Resource, Prompt, API Endpoint, curl, and AI View simulators invoke `/api/mcp/server/:shareSlug`; this keeps internal management identity separate from the public MCP transport identity.
- AI View and other generated MCP connection examples resolve their absolute URL from `VITE_API_URL` through `backendUrl()` when configured; `window.location.origin` is used only as the same-origin fallback.
...
Rate limiting is always active at a fixed 60 req/min per server and is not configurable.

- `RateLimitGuard` enforces the fixed limit on MCP endpoints, keyed per server (id and share slug share one window), returning HTTP 429 with `Retry-After` when exceeded. The stored per-server `rateLimit` config is ignored (legacy — see `docs/ENTITIES.md`).
- The Server Detail Harness tab shows the Request Limit panel as display-only: switch locked to Active, value locked to 60 req/min. There is no edit action, so no permission gates this panel (the former `PATCH /swagger/servers/:id/rate-limit` endpoint was removed).
