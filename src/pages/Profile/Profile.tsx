...
function MyProfileTab({ me, onUpdated }: MyProfileTabProps) {
  ...
  const handleSave = async () => {
    ...
    const attrs: { email?: string; password?: string; data?: { username: string } } = {}
    if (email !== me.email) attrs.email = email
    if (username !== me.username) attrs.data = { username }
    if (newPassword) attrs.password = newPassword

    if (Object.keys(attrs).length === 0) { showFeedback(t('myProfile.noChanges'), 'error'); return }
    ...
    try {
      ...
      showFeedback(
        attrs.email ? t('myProfile.saveSuccessEmailPending') : t('myProfile.saveSuccess'),
        'success',
      )
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : t('myProfile.saveError')
