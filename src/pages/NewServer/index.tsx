...
const SOURCE_TYPES: Array<{
  ...
  { id: 'graphql', name: 'GraphQL',   description: 'Expose queries and mutations from a GraphQL endpoint as MCP tools.', emoji: '🔮', color: '#E535AB', available: true, group: 'API' },
  { id: 'grpc',    name: 'gRPC',      description: 'High-performance RPC services using Protocol Buffer definitions.',    emoji: '⚡', color: '#fca130', available: true, group: 'API' },
  ...
  { id: 'mssql',       name: 'SQL Server',   description: 'Microsoft\'s enterprise relational database management system.',        emoji: '🏢', color: '#CC2927', available: true, group: 'SQL' },
  { id: 'oracle',      name: 'Oracle DB',    description: 'Oracle\'s enterprise-grade relational database.',                       emoji: '🏛️', color: '#F80000', available: true, group: 'SQL' },
  { id: 'cockroachdb', name: 'CockroachDB',  description: 'Distributed SQL database with PostgreSQL compatibility.',               emoji: '🪳', color: '#6933FF', available: true, group: 'SQL' },
  { id: 'clickhouse',  name: 'ClickHouse',   description: 'Column-oriented database for real-time analytics at scale.',            emoji: '🖱️', color: '#FAFF00', available: true, group: 'SQL' },
  // ── NoSQL / Document / Key-value ─────────────────────────────────────────
  { id: 'mongodb',       name: 'MongoDB',       description: 'Document-oriented NoSQL database with flexible schema.',              emoji: '🍃', color: '#47A248', available: true, group: 'NoSQL' },
  { id: 'redis',         name: 'Redis',          description: 'In-memory key-value store for caching, queues, and pub/sub.',        emoji: '🔴', color: '#DC382D', available: true, group: 'NoSQL' },
  { id: 'cassandra',     name: 'Cassandra',      description: 'Wide-column store for high availability and massive datasets.',       emoji: '👁️', color: '#1287B1', available: true, group: 'NoSQL' },
  { id: 'firestore',     name: 'Firestore',      description: 'Google\'s serverless document database for web and mobile apps.',    emoji: '🔥', color: '#FFCA28', available: true, group: 'NoSQL' },
  // ── Cloud / Warehouse ────────────────────────────────────────────────────
  { id: 'dynamodb',      name: 'DynamoDB',       description: 'AWS fully managed key-value and document database service.',         emoji: '🔷', color: '#232F3E', available: true, group: 'Cloud' },
  { id: 'elasticsearch', name: 'Elasticsearch',  description: 'Distributed search and analytics engine built on Apache Lucene.',    emoji: '🔍', color: '#FEC514', available: true, group: 'Cloud' },
  { id: 'snowflake',     name: 'Snowflake',      description: 'Cloud data warehouse with multi-cluster shared data architecture.',  emoji: '❄️', color: '#29B5E8', available: true, group: 'Cloud' },
]
...
const CONNECTION_MANAGED_SOURCES: SourceType[] = [
...
export default function NewServer() {
  ...
  const isSourceAvailable = (nextSourceType: SourceType | null) =>
    ...
    (!CONNECTION_MANAGED_SOURCES.includes(nextSourceType) || can(Permission.ServersManageConnection))
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (sourceType === 'rest') {
        ...
        projectId = data._id
        if (CONNECTION_MANAGED_SOURCES.includes(sourceType as SourceType)) {
          const connectionConfig = SQL_HOSTS.includes(sourceType as SourceType)
            ? { host: dbHost.trim(), port: Number(dbPort || DB_PORT_DEFAULTS[sourceType as SourceType]), database: dbDatabase.trim(), user: dbUser.trim(), password: dbPassword, ssl: dbSsl }
            : sourceType === 'mongodb'
              ? { uri: mongoUri.trim() }
              : sourceType === 'redis'
                ? { redisHost: redisHost.trim(), redisPort: Number(redisPort), redisPassword, redisTls }
                : sourceType === 'dynamodb'
                  ? { dynamoRegion, dynamoAccessKey, dynamoSecretKey, dynamoEndpoint: dynamoEndpoint.trim() || undefined }
                  : sourceType === 'elasticsearch'
                    ? { esUrl: esUrl.trim(), esApiKey, esUser: dbUser.trim(), esPassword: dbPassword }
                    : sourceType === 'snowflake'
                      ? { snowflakeAccount: snowflakeAccount.trim(), snowflakeWarehouse: snowflakeWarehouse.trim(), snowflakeSchema: snowflakeSchema.trim(), database: dbDatabase.trim(), user: dbUser.trim(), password: dbPassword }
                      : { firestoreProject: firestoreProject.trim(), firestoreCredentials }
          await api.patch(`/swagger/servers/${projectId}/connection`, connectionConfig)
        }
        // Auth (GraphQL / gRPC)
